...
export function getPosts(){if(!fs.existsSync(directory))return[];return fs.readdirSync(directory).filter(f=>/\.mdx?$/.test(f)&&!/^README\.md$/i.test(f)).map(parse).sort((a,b)=>b.date.localeCompare(a.date))}
